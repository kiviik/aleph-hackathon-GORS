...
superficie Expo Android-only con la misma navegación y lenguaje visual de la app
mobile anterior (`Mapa`, `Street View`, `Guardados`), permiso de cámara,
diagnóstico offline, trace local y perfil EAS para `.apk`. La captura real,
YOLO/ONNX y QVAC siguen detrás de gates de validación en un dispositivo físico;
mientras tanto el resultado es `REFUSE`, nunca un positivo simulado.
