...
iOS is not blocked architecturally (`@qvac/onnx` ships `ios-arm64` prebuilds with
CoreML), only logistically: it needs a Mac to build. `mobile/app.json`,
`mobile/eas.json` and the plugin are Android-only today.
