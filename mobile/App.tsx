 ...
 */
const spotTitle = (spot: Spot) => (spot.sideLabel ? `${spot.street} · ${spot.sideLabel}` : `${spot.street} ${spot.number}`);
...
export default function App() {
  ...
  const mapMarkers = useMemo(() => visibleSpots.map((spot) => ({
    ...
    meta: [spot.number, spot.spanM ? `~${Math.round(spot.spanM)} m of curb` : null, spot.accuracyM ? `±${spot.accuracyM} m` : null].filter(Boolean).join(" · "),
    curb: spot.curb ?? null,
