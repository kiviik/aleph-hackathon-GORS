...
function StreetView({ spot, spots, onSelect, darkMode, onToggleTheme }: { spot: Spot; spots: Spot[]; onSelect: (spot: Spot) => void; darkMode: boolean; onToggleTheme: () => void }) {
  // The Maps URL API is reliable in a normal browser but is frequently blank in an
  // embedded mobile WebView. The legacy `output=svembed` endpoint is still the
  // supported no-key embed surface, so try it first and keep the Maps URL as a
  // fallback for devices where Google has retired the legacy endpoint.
  const embedSource = `https://maps.google.com/maps?q=&layer=c&cbll=${spot.latitude},${spot.longitude}&cbp=11,${spot.heading},0,0,0&output=svembed&source=embed&hl=en`;
  ...
  useEffect(() => {
    // Google can leave an embedded panorama blank without emitting a WebView
    // error. Surface the in-app fallback instead of leaving an infinite spinner.
    const timer = setTimeout(() => setStreetViewError(true), 12000);
    return () => clearTimeout(timer);
  }, [spot.id, sourceIndex, webViewKey]);
  ...
  const source = sourceIndex === 0 ? embedSource : mapsSource;
  return (
    <View style={[styles.streetScreen, darkMode && styles.screenDark]}>
      <View style={styles.streetTopBar}>
        ...
        <View style={styles.headerActions}><ThemeToggle darkMode={darkMode} onPress={onToggleTheme} /><View style={[styles.livePill, darkMode && styles.darkPill]}><View style={styles.liveDot} /><Text style={[styles.liveText, darkMode && styles.textMutedDark]}>GOOGLE STREET VIEW</Text></View></View>
      </View>
      <View style={[styles.streetHeroCard, darkMode && styles.darkCard]}>
        ...
        <View style={styles.streetFrame}>
          {streetViewError ? <View style={styles.streetFallback}><MapView style={styles.streetFallbackMap} initialRegion={{ latitude: spot.latitude, longitude: spot.longitude, latitudeDelta: 0.004, longitudeDelta: 0.004 }} scrollEnabled={false} zoomEnabled={false} rotateEnabled={false} pitchEnabled={false} toolbarEnabled={false}><Marker coordinate={{ latitude: spot.latitude, longitude: spot.longitude }} pinColor={statusColor[spot.status]} /></MapView><View style={[styles.streetFallbackPanel, darkMode && styles.darkCard]}><Text style={[styles.webErrorIcon, darkMode && styles.textDark]}>◎</Text><Text style={[styles.webErrorTitle, darkMode && styles.textDark]}>Street View preview unavailable</Text><Text style={[styles.muted, darkMode && styles.textMutedDark]}>The panorama is blocked in this embedded browser. Your parking spot is still shown on the live map.</Text><View style={styles.streetFallbackActions}><Pressable onPress={reloadStreetView} style={styles.webRetry}><Text style={styles.webRetryText}>Try again</Text></Pressable><Pressable onPress={() => void Linking.openURL(mapsSource)} style={styles.webOpen}><Text style={[styles.webOpenText, darkMode && styles.textDark]}>Open Google Maps</Text></Pressable></View></View></View> : <WebView key={webViewKey} source={{ uri: source }} style={styles.webView} originWhitelist={["http://*", "https://*"]} javaScriptEnabled domStorageEnabled cacheEnabled thirdPartyCookiesEnabled setSupportMultipleWindows={false} allowsInlineMediaPlayback mediaPlaybackRequiresUserAction={false} mixedContentMode="always" userAgent={Platform.OS === "ios" ? "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1" : "Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36 Chrome/124.0.0.0 Mobile Safari/537.36"} startInLoadingState renderLoading={() => <View style={styles.webLoading}><ActivityIndicator color="#247b52" /><Text style={styles.muted}>Loading Street View…</Text></View>} onLoadStart={() => setStreetViewError(false)} onError={handleStreetViewError} onHttpError={handleStreetViewError} />}
  ...
  return (
    <View style={[styles.testingScreen, darkMode && styles.screenDark]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}><ThemeToggle darkMode={darkMode} onPress={onToggleTheme} /><View style={[styles.testingApiPill, darkMode && styles.darkPill]}><View style={styles.liveDot} /><Text style={[styles.testingApiText, darkMode && styles.textMutedDark]}>LOCAL FEED</Text></View></View>
      </View>
